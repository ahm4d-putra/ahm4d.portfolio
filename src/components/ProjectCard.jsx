...
export default function ProjectCard({ project, index }) {
  ...
  const getText = (key, fallback) => {
    ...
    // Kalau key yang direturn sama dengan input, berarti gak ada terjemahan
      ...
      <div className="relative h-full glass rounded-2xl p-8 hover:border-accent/30 transition-all duration-300 flex flex-col">
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            <h3 className="font-display text-2xl font-semibold text-light mb-1 group-hover:text-accent transition-colors">
              {/* Pakai helper getText */}
              {getText(`projects_${project.id}_title`, project.title)}
            </h3>
        ...
        <div className="space-y-4 mb-6 flex-grow">
          <div>
            ...
            <p className="text-light/80 text-sm mt-1 leading-relaxed">
              {getText(`projects_${project.id}_problem`, project.problem)}
            </p>
          </div>
          <div>
            ...
            <p className="text-light/80 text-sm mt-1 leading-relaxed">
              {getText(`projects_${project.id}_solution`, project.solution)}
            </p>
          </div>
        </div>
